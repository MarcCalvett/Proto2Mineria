...
namespace Fraktalia.VoxelGen.Visualisation
{
	...
	public class BasicNativeVisualHull : MonoBehaviour
	{
		...
		protected VoxelPiece CreateVoxelPiece(string Name, Material material, Material materialShell = null)
		{
			...
			GameObject newgameobject = newobj.gameObject;
			newgameobject.isStatic = gameObject.isStatic;
			newgameobject.layer = gameObject.layer;

			if (AppliedHideflags == HideflagsMode.Normal) newgameobject.hideFlags = HideFlags.None;
			if (AppliedHideflags == HideflagsMode.DontSave) newgameobject.hideFlags = HideFlags.DontSave;
			if (AppliedHideflags == HideflagsMode.Hidden) newgameobject.hideFlags = HideFlags.HideInHierarchy;
			if (AppliedHideflags == HideflagsMode.HiddenDontSave) newgameobject.hideFlags = HideFlags.HideAndDontSave;
